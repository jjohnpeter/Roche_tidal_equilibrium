...
def pltimg(ax, xarr, yarr, zarr, min_val, max_val, extend, xlabel, ylabel, zlabel, cmap,
    ...
    ax.set_xlabel(xlabel, labelpad=-2, fontsize=2)
    ax.set_ylabel(ylabel, fontsize=22)
    # old ax.set_xlabel(xlabel, labelpad=-2)
    # old ax.set_ylabel(ylabel)
    ...
    CB = pl.colorbar(im, ax=ax, ticks=CB_levels, extend=extend,
                     orientation='horizontal', pad=0.1, fraction=.046) # added pad, fract
    CB.ax.set_xticklabels(CB_ticklabels, fontsize=18) # added fontsize
    CB.ax.set_xlabel(zlabel, fontsize=22, labelpad=8) # added fontsize, label pad from 3-->8
    CB.ax.tick_params(axis='x', labelsize=18) # new line
    # CB.ax.set_yticklabels(CB_ticklabels)
    # CB.ax.set_ylabel(zlabel, labelpad=3)
    CB.ax.minorticks_off()
    ax.tick_params(axis='x', which='major', labelsize=18, width=1.8) # new line
    ax.tick_params(axis='y', which='major', labelsize=18, width=1.8) # new line
