...
def Phitidal_sma_spin(x, y, z, Qbh, qstar, sma, Omega_sq):
    mu=Qbh / (Qbh + qstar)
    # 1st term is the BH Grav, should be the same
    # 2nd term is the Centrifugal potential where the change is necessary: spin more from more of
    # star's mass toward BH
    return -Qbh/sqrt((x-sma)**2 + y**2 + z**2) - 0.5*(Omega_sq) * ((x-mu*sma)**2 + y**2)
...
#def WD_Radius(Mstar):
#    Rstar = 0.01125 * np.sqrt(((Mstar/1.454)**(-2/3)) - ((Mstar/1.454)**(2/3)))
#    return Rstar

def WD_Radius(Mstar):
    Rstar = 0.0127 * Mstar**(-1/3)
    return Rstar

def MS_Radius(Mstar):
    dir_main = '/Users/JovanJohnPeter/Roche_tidal_equilibrium/'
    savedir = dir_main + 'MS_K_runs/'
