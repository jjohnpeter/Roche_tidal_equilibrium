...
#npoly, ximax, phimax = 3.0, 6.89676, 2.01825   # obtained from LaneEmden.py

dir_main = '/Users/JovanJohnPeter/Roche_tidal_equilibrium/MS_Q_runs/'
...
for i in range(NK):
    ...
    rhocarr = prop[i, :, 5]
    #print(f'rhocarr = {rhocarr}')
    xsurfarr = prop[i, :, 3]
    #print(f'xsurfarr = {xsurfarr}')
    Phitotsurf = prop[i, :, 4]
    qstararr = prop[i, :, 0]
    #print(f'Qstar: {qstararr}')
    if max(qstararr) > qstar_target > min(qstararr):
        ...
        print('qstar_target out of simulated range: [%.5f, %.5f]' % (min(qstararr), max(qstararr)))
    for j in range(Nrhoc):
        ...
        rhoc = prop[i, j, 5]
        qstar = prop[i, j, 0]
        #if qstar < 0.01: # Variable threshold, change based on particular simulations
        #    continue  #
        ...
        ax.scatter(rhoc, Rstar/RLeff, s=40, marker='*', color='k', label=leg_label)
    ...
    sma_path = dir_main + f'sma{sma:.1f}/' + f'K{Kentr}'
    if not os.path.exists(sma_path):
        os.makedirs(sma_path, exist_ok=True)
    print(f'Save path: {sma_path + labels[kplt_list[-1]] + '.png'}')
    fig.savefig(sma_path + '/' + labels[kplt_list[-1]] + '.png', dpi=300)
