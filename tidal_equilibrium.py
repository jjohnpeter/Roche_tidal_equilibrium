...
#savedir_Krhoc = savedir + 'Kentr%.5f/x0surf%.5f/' % (Kentr, x0surf)    # where data and prints are saved
savedir_Krhoc = savedir + 'Kentr%.5f/rhoc%.3f/' % (Kentr, rhoc)   # where data and prints are saved
...
# run the C code
#run_command = srcdir + 'run %d %.3f %.3f %d %.3f %.5f %.5f' % (Niter, npoly, Lmax, Nresz, rhoc, Kentr, x0surf)
