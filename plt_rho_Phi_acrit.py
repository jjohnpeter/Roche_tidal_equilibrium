...
# for the fiducial run (after running run_Kentr_acrit.py), you can use (for example)
# python plt_rho_Phi.py python plt_rho_Phi_acrit.py 0 5 7905.777 0.133 0.90
...
rhoc = float(sys.argv[3])   # central density
sma = float(sys.argv[4])    # orbital separation
Qbh = float(sys.argv[5])    # BH, companion mass
Lmax = float(sys.argv[6])

#old paths
#savedir_Krhoc = savedir + 'Kentr%.5f/sma%.5f/' % (Kentr, sma)
#savedir_Krhoc = savedir + 'rhoc%.3f/sma%.5f/' % (Kentr, sma)
...
rhoCB_levels = [-7, -5, -3, -1, 0, 0.5, 1, 2, 3, 3.5, 4, 5, 5.5]
...
if plt_case == 0:
    ...
    if k_plt == 0:  # in the x-y plane
        ...
        # potential at L1
        PhiL1_val = Phitotarr[np.searchsorted(xarr, xL1), 0, 0] - Phitidal_0th
        PhiL2_val = Phitotarr[np.searchsorted(xarr, xL2), 0, 0] - Phitidal_0th
...
min_val = -50   # manually set min here
...
if plt_case == 0:
    ...
    # Plot Roche Lobe (equipotential contour through L1)
    ax.contour(X, Y, pltarr.transpose(), levels=[PhiL1_val], linestyles='--',
               colors='cyan', linewidths=2)
    ax.contour(X, Y, pltarr.transpose(), levels=[PhiL2_val], linestyles = '--',
               colors='red', linewidths=2)
    #CS = ax.contour(X, Y, logrhoarr.transpose(),
    #                levels = [np.log10(eps_small)], linestyles='solid',
    #                colors='green', linewidths=2, alpha=1)
    fmt = {}
